...
# Workflow is an object that contains the model specification
...
# fit_resamples() evaluates the model using cross-validation
results <- workflow %>%
  fit_resamples(cv, metrics = metric_set(accuracy))

# Extract the metrics from the results
# collect_metrics() extracts the performance metrics from the results
# filter() keeps just the accuracy
metrics <- results %>%
  collect_metrics() %>%
...
# std deviation = std error * sqrt(sample size)
# where sample size is the number of observations (in this case, number of repetitions)
n <- n_folds * n_repeats
